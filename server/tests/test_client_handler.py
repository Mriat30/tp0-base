...
class TestClientHandler(unittest.TestCase):
    ...
    def test_handle_single_bet_success(self):
        ...
        self.mock_lottery.store_bets.assert_called_once_with([bet])
    ...
    def test_handle_batch_fails_if_storage_fails(self):
        ...
        self.mock_lottery.store_bets.side_effect = Exception("Storage error")
        self._prepare_proto(OpCode.REGISTER_BATCH_OF_BETS, bets, is_batch=True)
        
        self.handler.start()
        
        self.mock_lottery.store_bets.assert_called_once_with(bets)
